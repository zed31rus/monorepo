...
const toggleIcon = computed(() =>
	isOpen.value ? 'line-md:menu-to-close-alt-transition' : 'line-md:close-to-menu-alt-transition'
);
...
const topPages = computed(() => pages.filter((p) => p.position === 'top'));

const bottomPages = computed(() => pages.filter((p) => p.position === 'bottom'));
	...
	<div ref="scope">
		<div ref="menuWrapper" class="menu-wrapper">
			<button @click="toggle" class="toggle-button">
				<Icon :icon="toggleIcon" :key="isOpen ? 'open' : 'closed'" class="toggle-icon" />
			</button>
					...
					<AnimatePresence>
						<MotionSideBarItem
							...
							:animate="{
								opacity: 1,
								x: 0,
								transition: { delay: page.id * 0.05, duration: 0.25 },
							}"
