...
export default class UsersExternalModule extends BaseModule<UsersMainEnv> {
	init() {
		this.router.use(this.wrappers.rateLimiter.limit(15 * 60 * 1000, 100));

		this.router.openapi(this.openapis.external.users.getByUuid, async (c) => {
			const { uuid } = c.req.valid('param');
			const user = await this.core.services.users.getByUuid(uuid, false);
			return c.json({ user });
		});

		this.router.openapi(this.openapis.external.users.getByEmail, async (c) => {
			const { email } = c.req.valid('param');
			const user = await this.core.services.users.getByEmail(email);
			return c.json({ user });
		});

		this.router.openapi(this.openapis.external.users.getByLogin, async (c) => {
